...
class Mem4PyAdapter:
    ...
    def run_coupling(self):
        ...
        while self.interface.is_coupling_ongoing():
            ...
            with open("max_displacement_y.txt", "ab") as f:
                f.write(b"\n")
                np.savetxt(f, np.r_[[self.dt *self.iteration], [max(self.displacements[:,1].tolist(), key=abs)]], delimiter =',', newline=' ')
            
            
            
            # always return 0 displacement to OpenFOAM
            self.displacements[:, 0] = 0 #np.asarray(self.solver.X) -np.asarray(self.solver.X0)
            self.displacements[:, 1] = 0 #np.asarray(self.solver.Y) -np.asarray(self.solver.Y0)
            self.displacements[:, 2] = 0 #np.asarray(self.solver.Z) -np.asarray(self.solver.Z0)
