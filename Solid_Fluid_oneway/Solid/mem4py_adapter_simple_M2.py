...
class Mem4PyAdapter:
    ...
    def run_coupling(self):
        ...
        while self.interface.is_coupling_ongoing():
            ...
            # run solver
            self.solver.Sx = np.zeros(len(self.solver.elFSI), dtype=np.double)    #np.copy(self.read_data[:,0]) 
            self.solver.Sy = np.ones(len(self.solver.elFSI), dtype=np.double)* self.iteration * 0.0001    #np.copy(self.read_data[:,1])
            self.solver.Sz = np.zeros(len(self.solver.elFSI), dtype=np.double)   #np.copy(self.read_data[:,2])
